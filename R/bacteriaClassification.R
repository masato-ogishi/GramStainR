...
bacteriaClassification <- function(trainDF, testDF, validDF=NULL, 
  ...
  View(df_lb)
  best_model <- h2o::h2o.getModel(dplyr::filter(df_lb, !stringr::str_detect(model_id, "StackedEnsemble"))[["model_id"]][[1]])
  ...
  predEval <- function(evalH2ODF, H2OMod, withReference=T, predictType=c("Bacteria","Image"), outputHeader=""){
    ...
    predDF <- data.frame("PredictedBacteria"=predDF[[1]], "Probability"=predDF[[2]])  ## binary classification only
    ...
    if(withReference==F){
      ...
      if(predictType=="Bacteria"){
        ...
        predDF <- predDF %>% 
          ...
          dplyr::summarise(PredictedBacteria=which.max(table(PredictedBacteria)), Probability=mean(Probability)) %>%
      ...
      if(predictType=="Bacteria"){
        ...
        predDF <- predDF %>% 
          dplyr::group_by(Source) %>% 
          dplyr::summarise(Bacteria=unique(Bacteria), PredictedBacteria=which.max(table(PredictedBacteria)), Probability=mean(Probability)) %>%
          dplyr::mutate(PredictedBacteria=factor(PredictedBacteria, levels=lev))
        sink(paste0(outputHeader, "/ConfusionMatrix.txt"))
        print(caret::confusionMatrix(predDF$"PredictedBacteria", predDF$"Bacteria"))
        sink()
